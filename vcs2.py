...
def actionMax(total):
    return max(Q[total[0], total[1], total[2], 0], Q[total[0], total[1], total[2], 1], Q[total[0], total[1], total[2], 2], Q[total[0], total[1], total[2], 3])

def argActionMax(total):
    return np.argmax(np.array([Q[total[0], total[1], total[2], 0], Q[total[0], total[1], total[2], 1], Q[total[0], total[1], total[2], 2], Q[total[0], total[1], total[2], 3]]))
    ...
    for step in range(end):
        ...
        if t == 0 and testing == False:
            ...
            for curr in waiting_cars:
                total_score += traci.vehicle.getAccumulatedWaitingTime(curr)
            ...
            if random.uniform(0, 1) < epsilon:
                ...
                action = argActionMax(curr_state) # best action (exploit)
            ...
            if step > 0:
                ...
                Q[prev_state] = Q[prev_state] + lr * (reward + gamma * actionMax(total_state) - Q[prev_state])
        ...
        if t == 0 and testing == False:
            base_ys[step] = pcount
            enter_ys[step] = cum_car
        else: 
            ys[step] = pcount
    traci.close()
    np.save('q', Q)

plt.plot(xs, enter_ys, label='Car distribution')
